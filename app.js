...
app.get('/proxy', async (req, res) => {
	...
	try {
		...
		if (/^(image|audio|video|application\/(pdf|zip|octet-stream|x-msdownload|x-executable|x-rar-compressed|x-7z-compressed|apk))/.test(contentType)) {
			...
			html = html.replace(/(src|href|action)=["']([^"']+)["']/g, (match, attr, value) => {
				if (value.startsWith('http://') || value.startsWith('https://')) {
					return `${attr}="/proxy?url=${encodeURIComponent(value)}"`;
				} else {
					const resolvedUrl = new URL(value, baseHref).href;
					return `${attr}="/proxy?url=${encodeURIComponent(resolvedUrl)}"`;
				}
			});
