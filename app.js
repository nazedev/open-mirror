...
app.get('/screenshot', async (req, res) => {
	const { url, preset = 'desktop', fullpage = 'false', width, height } = req.query;
	if (!url) return res.status(400).send('Missing ?url=');
	try {
		const viewports = {
			...
			android: { width: 360, height: 740, userAgent: 'Mozilla/5.0 (Linux; Android 10; Mobile) AppleWebKit/537.36 Chrome/91.0.4472.77 Mobile Safari/537.36' },
		};
		...
		if (preset === 'android' && viewports.android.userAgent) {
			await context.setDefaultUserAgent(viewports.android.userAgent);
		}
